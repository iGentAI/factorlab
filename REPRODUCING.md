...
crash inside the extension rather than fail. Run the tests with `python -m pytest -q` (366 tests, about a minute; see `tests/README.md`). All commands
...
The commands below write where their `--out` says, and most drivers default `--out` to the very archive under `results/` that their
row cites (`arms_e45`, `balanced_structure`, `cm_asymmetry`, `common_factor`, `common_factor_curve`, `cover_search`, `energy_stats`,
`farey_cover`, `harvey_residue`, `lehman_product`, `modfree_census`, `nondriftfree_census`, `order_selection`, `schnorr_lattice`,
`sidon_bucketed`, `theorem_checks`; `latticelab.audit_detection`, `certify_audit`, `qceiling`, `simulator_chain`, `uniform_slack`),
while the `run_*` drivers (`run_hitting`, `run_beyond`, `run_frontier`, `run_fixed_list`, `run_fixed_scale`, ...), `sidon_scaling`,
`planar_census` and `resonant_census` write fixed file names under `results/`. Running any of them in a checkout therefore overwrites
a published archive: bit-identically when the run reproduces, and otherwise not (a `--quick` run, a reduced parameter list or a
machine-dependent timing field all change the file). Regenerate in a scratch copy of the repository, or pass `--out` to a path
outside `results/` where the command accepts it, and restore anything overwritten from version control (`git checkout -- results/`).
Where an archive was assembled from several runs, the command column lists every run; the list-valued flags of `energy_stats` and
`pileup_certify` take explicit values and reject a bare flag.
...
| two-sided crossings 418/654/924 ($\varepsilon = 0$), 414/649/917 ($0.01$), 406/638/903 ($0.03$), first passing in the scanned range | `dual_floor.two_sided_beta_floor` | `results/lattice_two_sided_scan.json`, `results/lattice_two_sided_scan.log`, `results/lattice_two_sided_scan_eps.log` | `python -m latticelab.dual_floor --d 1003 --beta-spec 403 --lo 410 --hi 430 --eps 0 --dual-mode full --out results/lattice_two_sided_scan.json`; `--lo 640 --hi 670` and `--lo 905 --hi 940` for the other sets; at slack, `--eps 0.01 --lo 409 --hi 421` (640--658, 902--926) and `--eps 0.03 --lo 401 --hi 413` (629--647, 889--913). The three $\varepsilon = 0$ rows of the archive were written by an earlier revision of `dual_floor` and carry the keys `beta_floor_two_sided`, `rhf_two_sided`, `shift_l1`, `complete`, `witness`; the six slack rows, and the current code, write `beta_floor`, `rhf_two_sided_lower`, `shift`, `prec`, `minimality`. The values agree. |
...
| balanced / primitive / unique-product census at nineteen moduli (seed 5: two at each even size from 30 to 46 bits and one at 48, the moduli of the report's Table of moduli), own-window statistics, Lemma D values | `factorlab.experiments.balanced_structure.census_point`, `family_stats` | `results/e53_balanced_structure.json` (every modulus listed; assembled from two runs, see `runs`), log `results/e53_balanced_structure.log` | `python -m factorlab.experiments.balanced_structure --bits 30 32 34 36 38 40 42 44 46 --count 2 --out results/e53_balanced_structure.json` then `python -m factorlab.experiments.balanced_structure --bits 48 --count 1 --out results/e53_balanced_structure.json` (the second run merges its row into the archive by modulus; 13 min in all, under 1 GB: above 10^8 pairs the difference scans run in value buckets, and the energy form is computed only for families with at most 5·10^7 pairs -- every family at 30--42 bits, the balanced, primitive and unique-product families at 44 bits and the primitive and unique-product families at 46 -- and recorded as not computed elsewhere; the report quotes the energy form at 30--42 bits only); any single modulus $N$ of the archive by `census_point(N, C=2, seed=5)` |
...
| half-offset census (one modulus at 42 and at 48 bits) | `balanced_structure.half_offset_census` | `results/e53_half_offset.json` | `python -m factorlab.experiments.balanced_structure --half-offset --bits 42 48 --count 1 --out results/e53_half_offset.json` |
| excision census, $M = 3, 4, 6, 8$ (one modulus at 42 and at 48 bits) | `balanced_structure.excision_census` | `results/e53_excision.json` | `python -m factorlab.experiments.balanced_structure --excision --bits 42 48 --count 1 --out results/e53_excision.json` |
...
| boundary certification of the exact maxima; pooled maxima; out-of-sample radii; 200-bit rechecks of the E39 census families and of the E27 symmetric families | `factorlab.experiments.pileup_certify.certify_boundary`, `out_of_sample`, `modfree_census.pooled_cluster` (via `--pool`), `pileup_certify.pooled_high_precision`, `recheck_census`, `symmetric_family_recheck` | `results/e41_certify_oos.json` (keys `certify`, `oos`), `results/e41_pooled.json` (keys `pool`, `recheck`, `pool_hp`), `results/e41_e27_recheck.json` (a list, one row per (r, j)) | `python -m factorlab.experiments.pileup_certify --certify 65536 131072 262144 --oos 150000 200000 --out results/e41_certify_oos.json`; `python -m factorlab.experiments.pileup_certify --pool 65536 262144 1048576 4194304 16777216 67108864 268435456 1073741824 --recheck --pool-hp 67108864 1073741824 --out results/e41_pooled.json`; `python -m factorlab.experiments.pileup_certify --recheck-e27 14:9 15:9 16:15 18:15 20:21 22:45 24:75 26:105 28:165 30:255 32:405 --out results/e41_e27_recheck.json` (under a minute) |
| additive energy against the null at $r = 2^{12} \ldots 2^{15}$; planar energy form at 40 bits (radii `third`, `crossing`, `quarter`) and 48 bits (`crossing`, `quarter`) | `factorlab.experiments.energy_stats.modfree_energy`, `planar_top_mass` | `results/e42_energy.json` (keys `energy`, `planar`; its `census_bigq` and `class_counts` keys hold other runs merged into the same file and are not written by `energy_stats`) | `python -m factorlab.experiments.energy_stats --energy 12 13 14 15 --planar 40 --planar-r third,crossing,quarter --out results/e42_energy.json`, then `python -m factorlab.experiments.energy_stats --planar 48 --planar-r crossing,quarter --out results/e42_energy.json` (rows are merged into the archive by radius and by (bits, radius)) |
...
| balanced, unique-product, half-offset and excision censuses; the maximisers | `balanced_structure` | `results/e53_balanced_structure.json`, `results/e53_uniq_maximisers.json`, `results/e53_half_offset.json`, `results/e53_excision.json` | `python -m factorlab.experiments.balanced_structure --bits 30 32 34 36 38 40 42 44 46 --count 2 --out results/e53_balanced_structure.json` then `... --bits 48 --count 1 --out results/e53_balanced_structure.json` (merged by modulus); `python -m factorlab.experiments.balanced_structure --maximisers --moduli 880634351 964728493 3722008519 3601086119 16577631001 14048619007 46551225037 51465965107 197762968751 187452546833 876190896151 680093802697 3890830613443 3072789281321 11427345574369 16247581009873 55862770399391 46686914783221 176552314063291 --out results/e53_uniq_maximisers.json`; `python -m factorlab.experiments.balanced_structure --half-offset --bits 42 48 --count 1 --out results/e53_half_offset.json`; `python -m factorlab.experiments.balanced_structure --excision --bits 42 48 --count 1 --out results/e53_excision.json` |
